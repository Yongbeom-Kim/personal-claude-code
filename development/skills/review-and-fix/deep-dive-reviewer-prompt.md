# Deep Dive Reviewer Prompt Template

Use this template when dispatching the deep dive correctness reviewer subagent.

Purpose: Deep analysis of each atomic change and its upstream/downstream dependencies. Spawns Explore sub-subagents per atomic change to trace logic, callers, and dependencies.
...
Task tool (general-purpose):
  ...
  prompt: |
    ...
    You also have access to the Agent tool to spawn `Explore` subagents.
    ...
    **Implementation plan (if available):** [PLAN_FILE_PATH or "N/A"]
    ...
    For EACH atomic change, spawn an `Explore` subagent (subagent_type: "Explore") in parallel. All Explore subagents run simultaneously — there is no limit.
